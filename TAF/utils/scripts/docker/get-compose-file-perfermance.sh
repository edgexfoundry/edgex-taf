...
# # Consul or Keeper
[ "$REGISTRY_SERVICE" = "Keeper" ] && USE_KEEPER="-keeper"

# # first sync standard docker-compose file from edgex-compose repo
./sync-compose-file.sh "${USE_SHA1}" "${USE_NO_SECURITY}" "${USE_ARM64}" "-taf" "${USE_KEEPER}" "-perf"

# # docker-compose.yml file is used on Case 1-4
cp docker-compose-taf-perf${USE_NO_SECURITY}${USE_KEEPER}${USE_ARM64}.yml docker-compose.yml
...
cp docker-compose-taf-perf${USE_NO_SECURITY}${USE_KEEPER}${USE_ARM64}.yml docker-compose-mqtt.yml
