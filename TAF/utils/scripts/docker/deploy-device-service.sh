...
if [ "$PROFILE_DIR" = "service_default" ]; then
  docker run --rm -v ${WORK_DIR}:${WORK_DIR}:rw,z -w ${WORK_DIR} -v /var/run/docker.sock:/var/run/docker.sock \
         ...
         ${COMPOSE_IMAGE} -f "${WORK_DIR}/TAF/utils/scripts/docker/docker-compose.yaml" up --no-start --no-deps $SERVICE_NAME
  ...
  docker run --rm -v ${WORK_DIR}:${WORK_DIR}:rw,z -w ${WORK_DIR} -v /var/run/docker.sock:/var/run/docker.sock \
         --security-opt label:disable  \
         --env WORK_DIR=${WORK_DIR} --env PROFILE=${PROFILE}/res --env CONF_DIR=${CONF_DIR} \
         ${COMPOSE_IMAGE} -f "${WORK_DIR}/TAF/utils/scripts/docker/docker-compose.yaml" start $SERVICE_NAME
else
