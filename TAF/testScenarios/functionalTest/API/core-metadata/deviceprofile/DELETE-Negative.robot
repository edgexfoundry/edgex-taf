...
ErrProfileDELETE004 - Delete device profile by name when StrictDeviceProfileDeletes is true
    Given Set ProfileChange.StrictDeviceProfileDeletes to true For Core-Metadata On Registry Service
    ...
    And Should Return Content-Type "application/json"
    And Response Time Should Be Less Than "${default_response_time_threshold}"ms
    [Teardown]  Run Keywords  Set ProfileChange.StrictDeviceProfileDeletes to false For Core-Metadata On Registry Service
    ...                  AND  Delete Device Profile By Name  Test-Profile-2
