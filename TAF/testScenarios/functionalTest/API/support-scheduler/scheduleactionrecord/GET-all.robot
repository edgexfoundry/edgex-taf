...
*** Variables ***
${SUITE}          Support Scheduler Action Record Get All Test Cases
${LOG_FILE_PATH}  ${WORK_DIR}/TAF/testArtifacts/logs/support-scheduler-action-record-get-all.log

*** Test Cases ***
SchedulerActionGET001 - Query all schedule action record
...
SchedulerActionGET002 - Query all schedule action record with start
...
SchedulerActionGET003 - Query all schedule action record with end
    ${currentTime}  Get Current Milliseconds Epoch Time
...
SchedulerActionGET004 - Query all schedule action record with start/end
    ${currentTime}  Get Current Milliseconds Epoch Time
...
SchedulerActionGET005 - Query all schedule action record with offset
...
SchedulerActionGET006 - Query all schedule action record by start and limit
    ${currentTime}  Get Current Milliseconds Epoch Time
    ${time}  Evaluate  ${currentTime}+1400
...
Query All Schedule Action Record By Parameters
    ...
    ${resp}  GET On Session  Support Scheduler  ${scheduleActionRecordUri}/all  params=${params_dict}
