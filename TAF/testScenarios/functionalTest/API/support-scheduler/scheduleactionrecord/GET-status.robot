...
*** Variables ***
${SUITE}          Support Scheduler Action Record Get By Status Test Cases
${LOG_FILE_PATH}  ${WORK_DIR}/TAF/testArtifacts/logs/support-scheduler-action-record-get-status.log

*** Test Cases ***
SchedulerActionStatusGET001 - Query schedule action record by status
...
SchedulerActionStatusGET002 - Query schedule action record by status with start
...
SchedulerActionStatusGET003 - Query schedule action record by status with end
    ${currentTime}  Get Current Milliseconds Epoch Time
...
SchedulerActionStatusGET004 - Query schedule action record by status with start/end
    ${currentTime}  Get Current Milliseconds Epoch Time
...
SchedulerActionStatusGET005 - Query schedule action record by status with offset
...
SchedulerActionStatusGET006 - Query schedule action record by status with limit
...
Query Schedule Action Record By Status With Parameters
    ...
    ${resp}  GET On Session  Support Scheduler  ${scheduleActionRecordUri}/status/${status}  params=${params_dict}
