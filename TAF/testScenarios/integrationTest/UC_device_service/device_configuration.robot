...
Config003 - Verify the return value after writing and reading with shift field set when Data Transform is true
# Use RW (with shift) to write and RW (with shift) to read. Value is transformed during both write and read.
    ${set_data}  Create Dictionary  Modbus_DeviceValue_UINT32_RW=4294901760
    Given Set Test Variable  ${PREFIX}  Modbus
    And Set Test Variable  ${device_name}  modbus_device
    And Create Device For device-modbus With Name Transform-Device-1
...
Config004 - Verify the return value after reading with shift field set when Data Transform is false
...
Config005 - Verify the return value after writing with shift field set when Data Transform is false
# Use RW (with shift) to write and R (without shift) to read. No value is transformed.
    ${set_data}  Create Dictionary  Modbus_DeviceValue_UINT32_RW=4294901760
    Given Set Device DataTransform to false For device-modbus On Registry Service
    And Set Test Variable  ${PREFIX}  Modbus
    And Set Test Variable  ${device_name}  modbus_device
...
Config006 - Verify the return value after writing and reading with shift field set when Data Transform is false
# Use RW (with shift) to write and RW (with shift) to read. No value is transformed.
    ${set_data}  Create Dictionary  Modbus_DeviceValue_UINT32_RW=4294901760
    Given Set Device DataTransform to false For device-modbus On Registry Service
    And Set Test Variable  ${PREFIX}  Modbus
    And Set Test Variable  ${device_name}  modbus_device
...
Config007 - Verfiy reading contains units when ReadingUnits is true
    [Tags]  backward-skip
...
Config008 - Verfiy reading contains units when ReadingUnits is false
    [Tags]  backward-skip
...
Config009 - Verify OperationalState when AllowedFails is default with failed device requests
...
Config010 - Verify OperationalState when AllowedFails is 1 and DeviceDownTimeout is default with failed device requests
...
Config011 - Verify OperationalState when AllowedFails is 1 and DeviceDownTimeout=1 with failed device requests
    ${set_data}  Create Dictionary  Modbus_DeviceValue_INT16_RW=60
    Given Set Test Variable  ${device_name}  Modbus-Test-Device
    And Create Unavailable Modbus Device
    And Set Device AllowedFails to 1 For device-modbus On Registry Service
