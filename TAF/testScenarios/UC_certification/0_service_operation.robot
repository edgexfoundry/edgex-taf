...
${WORK_DIR}           %{WORK_DIR}
${PROFILE}            %{PROFILE}
${LOG_FILE}           ${WORK_DIR}/TAF/testArtifacts/logs/service_operation.log
...
#   Deploy services  data  metadata  command  ${TEST_SERVICE}

Shutdown "${service_name}" service and Modify Configuration Service.ConnectRetries = 1 and Restart DS
    Stop services  ${service_name}
    Modify consul config  /v1/kv/edgex/devices/1.0/device-virtual/device-virtual/Service/ConnectRetries  1
    Restart services  ${TEST_SERVICE}

Restart EdgeX
    Deploy services  data  metadata  command  device-virtual
...
Run Test Echo
    FOR    ${item}    IN    @{SUPPORTED_DATA_TYPES}
      ${res} =  Given check value range  127  ${item["dataType"]}
      Should Be True  ${res}
    END
