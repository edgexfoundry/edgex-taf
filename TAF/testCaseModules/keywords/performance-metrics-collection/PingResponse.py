...
def send_ping_request(port, token):
    ...
    ping_url = "{}/api/{}/ping".format(service_url, SettingsInfo().constant.API_VERSION)
    header = {"Authorization": "Bearer {}".format(token)}
    res = requests.get(ping_url, headers=header, verify=False)
    logger.info("response body: {}, response time: {}".format(res.content, res.elapsed.total_seconds()))
    response = {
