...
run_compose() {
  ...
  docker run --rm \
        ...
        --security-opt label:disable ${COMPOSE_IMAGE} docker compose  --env-file compose.env -p edgex "$@"
}
...
if [ "${MACHINE}" = "edgex" ]; then
  ...
  curl -o compose_files/docker-compose.yml "${BUILD_URL}/edgex-compose/${USE_SHA1}/docker-compose${USE_NO_SECURITY}${USE_ARM64}.yml"
  ...
  do
    ...
    if [ "$SECURITY_SERVICE_NEEDED" = "true" ]; then
      sed -i "/message-bus/s/$/,postgres[app-mqtt-export_${n}],message-bus[app-mqtt-export_${n}]/" compose_files/docker-compose-add.yml
      sed -i "/EDGEX_ADD_SECRETSTORE_TOKENS/s/$/,app-mqtt-export_${n}/" compose_files/docker-compose-add.yml
    fi
  ...
  # Deploy services
  run_compose -f compose_files/docker-compose.yml -f compose_files/docker-compose-add${USE_NO_SECURITY}.yml up -d
  ...
  run_compose -f compose_files/docker-compose.yml -f compose_files/docker-compose-add${USE_NO_SECURITY}.yml \
              -f compose_files/docker-compose-simulators.yml -f compose_files/docker-compose-export.yml down -v
