...
try:
    ...
    print("Error running EventReceiver.py:", e.stderr)
    sys.exit(1)
...
try:
    ...
    print("Error initializing InfluxDB client:", str(e))
    sys.exit(1)

# Retrieve data from influxdb
# Generate Container CPU/Memory Usage DataFrame For LineChart
def query_safe(flux_query):
    try:
        return client.query_api().query_data_frame(flux_query)
    except Exception as e:
        print("Error querying InfluxDB:", str(e))
        return pd.DataFrame()

container_cpu_usage_flux = util.query_flux(bucket, time_range, "docker_container_cpu", "usage_percent", None, "1m", additional_flux="""
    |> keep(columns: ["container_name", "_value", "_time"])
    |> pivot(rowKey:["_time"], columnKey: ["container_name"], valueColumn: "_value")""")

container_mem_usage_flux = util.query_flux(bucket, time_range, "docker_container_mem", "usage", None, "1m", additional_flux="""
    |> map(fn:(r) => ({r with _value: r._value / 1024.0 / 1024.0}))
    |> keep(columns: ["container_name", "_value", "_time"])
    |> pivot(rowKey:["_time"], columnKey: ["container_name"], valueColumn: "_value")""")

container_cpu_usage = query_safe(container_cpu_usage_flux)
container_mem_usage = query_safe(container_mem_usage_flux)

if container_cpu_usage.empty or container_mem_usage.empty:
...
container_flux = util.query_flux(bucket, time_range, "docker_container_mem", "usage", None, "1m", additional_flux="""
        |> keep(columns: ["container_name"])
        |> distinct(column: "container_name")""")
services = query_safe(container_flux)
df = pd.DataFrame(services)
services = df['container_name'].tolist()
...
for service in service_list:
    ...
    for label in agg_labels:
        ...
        |> yield(name: "{label}")
"""
        data = query_safe(container_cpu_usage_frame_influx)
...
for service in service_list:
    ...
    for label in agg_labels:
        ...
        |> yield(name: "{label}")
"""
        data = query_safe(container_mem_usage_frame_influx)
...
for service in service_list:
    ...
    if 'edgex-' in service:
        if 'app-' in service:
            processed_list.append('-'.join(parts[1:]))  # Keep everything except the first part
        elif len(parts) <= 2:
            processed_list.append(parts[-1])  # Keep only the last part if length is <= 2
        else:
            processed_list.append('-'.join(parts[-2:]))  # Keep only the last two parts
    else:
        processed_list.append(service)  # Keep original

service_name_mapping = {orig: clean for orig, clean in zip(service_list, processed_list)}
